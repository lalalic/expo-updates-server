...
async function putRollBackInResponseAsync(
	...
	if (currentUpdateId === embeddedUpdateId) {
		throw new req.storage.NoUpdateAvailableError();
	}

	await send({req, res,directive:await req.storage.createRollBackDirectiveAsync(updateBundlePath)})
}
...
async function putNoUpdateAvailableInResponseAsync(req, res, protocolVersion) {
	...
	await send({res,req,directive:await req.storage.createNoUpdateAvailableDirectiveAsync(),})
}
